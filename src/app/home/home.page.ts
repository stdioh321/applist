...
export class HomePage implements OnInit {
  ...
  filterList(src = "") {
    ...
    this.applist = this.applist.map(((item: any) => {
      if ((item.package + "").toLowerCase().trim().indexOf(src) > -1 || (item.label + "").toLowerCase().trim().indexOf(src) > -1) {
        item.show = true;
      }
      else
        item.show = false;
  ...
  goDetail(app = null) {
    ...
    // console.log(app);
    this.router.navigate(['detail/' + app['package']], {relativeTo: this.aRoute});
  }
}
